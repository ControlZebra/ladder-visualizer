...
- **2026-09-18 — Issue #39 implementation:** The first issue #15 child slice pins the v33-v35 FBD compatibility boundary. Exactly one untagged static body is accepted; repeated bodies and every tagged or unknown online-edit state fail before normalization with `UNSUPPORTED_FBD_ONLINE_EDIT`, an owning routine path, and deterministic observed-state evidence.
- **2026-09-18 — Issue #40 implementation:** Accepted static Program-owned and AOI-owned FBD bodies now normalize into one canonical routine body. The model preserves ordered sheets, metadata provenance, positioned elements, arrays, AOI bindings, routine-control parameters, connections, annotations, and attachments; recoverable unsupported or malformed elements survive as diagnostic placeholders.
- **2026-09-19 — Issue #41 implementation:** FBD Blocks, supported Function extensions, and AOIs now resolve explicit wire-facing ports through controller/version-aware metadata. The slice covers all nine level-control mnemonics, DEDT storage-array requirements, ordered AOI Input/Output ports, separately preserved InOut bindings, and diagnosed placeholders for unknown or unusable metadata.
- **2026-09-19 — Issue #42 implementation:** The core FBD renderer now measures and places source-anchored elements, resolves routine-scoped connectors, routes normal and feedback wires deterministically, and renders IRef/ORef/ICon/OCon, generic Blocks, and array bindings. A sanitized two-sheet v35 level-control fixture exercises all nine Blocks, eleven normal wires, two feedback wires, and the `TankLevel` connector relationship with light/dark Chromium baselines.
- **2026-09-20 — Issue #63 implementation:** FBD Block ports now come from the resolved operand's own decorated direct-member structure instead of a mnemonic catalog. Program tags shadow Controller tags, AOI-owned routines use LocalTag default data, `EnableIn`/`EnableOut` delimit sides, visible pins filter exact member IDs, explicit empty pin lists remain valid, and source arrays are preserved without required-array rules. Function metadata now has a Function-only public API; the broad catalog names remain deprecated aliases for one compatibility cycle.
