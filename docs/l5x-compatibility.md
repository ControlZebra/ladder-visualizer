...
Accepted static bodies normalize to `NormalizedRoutine.fbd` for Program-owned and AOI-owned routines. The model retains ordered sheets and descriptions, exact string IDs/coordinates, references, connectors, blocks and arrays, AOI bindings, JSR/SBR/RET calls, wires, feedback wires, text boxes, and attachments. Implicit IRef/ICon source terminals and ORef/OCon destination terminals use the canonical `value` port. Blocks for ADD, DEDT, HLL, LDLG, MUL, PIDE, SUB, D2SD, and GRT resolve `VisiblePins` through a dedicated FBD catalog with exact IDs, labels, direction, side, order, default visibility, and DEDT array requirements. That existing metadata applies to Logix major versions v17 through v35 inclusive; this does not expand the separately pinned v33-v35 XSD conformance matrix. AOI Input and Output ports resolve from ordered parameter definitions while InOut bindings remain separate. Supported-controller ADD, MUL, SUB, and GRT Function extensions resolve from the same version-aware layer; other schema-absent Functions, unknown mnemonics, invalid metadata, and malformed/unplaceable nodes survive as placeholders with stable diagnostics and make the result partial. Missing presentation metadata uses explicit fallback provenance and informational diagnostics. The raw accepted body remains a `source-representation` fragment rather than an `unmodeled` fragment. See the [compatibility boundary](l5x-fbd-compatibility-slice.md), [normalization slice](l5x-fbd-normalization-slice.md), and [port-metadata slice](l5x-fbd-port-metadata-slice.md).
