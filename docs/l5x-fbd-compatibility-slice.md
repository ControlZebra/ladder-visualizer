...
- **Normalized result and invariants:** This slice deliberately leaves FBD body normalization to issue #40. Accepted static bodies retain the current routine header plus their complete raw document fragment. Rejected bodies return no controller or document data.
...
`DescriptionType` permits direct mixed text, `Value`, and localized-description forms. Real FBD sheet descriptions in the available corpus use CDATA. Exact description normalization belongs to issue #40; this slice keeps the whole accepted body as a parsed source fragment.
